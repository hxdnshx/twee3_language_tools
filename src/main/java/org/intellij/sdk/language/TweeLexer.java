 ...
 */
public class TweeLexer extends BaseTweeLexer {
  ...
  public TweeLexer() {
    this(new MergingLexerAdapter(new FlexAdapter(new _TweeLexer()), TOKENS_TO_MERGE), true, false);
  }

  public TweeLexer(boolean highlightMode) {
    this(new MergingLexerAdapter(new FlexAdapter(new _TweeLexer()), TOKENS_TO_MERGE), true, highlightMode);
  }

  protected TweeLexer(@NotNull Lexer baseLexer, boolean caseInsensitive) {
    this(baseLexer, caseInsensitive, false);
  }

  protected TweeLexer(@NotNull Lexer baseLexer, boolean caseInsensitive, boolean highlightMode) {
  ...
  public @Nullable IElementType getTokenType() {
    if (embeddedLexer != null) {
      ...
      if (tokenType == XmlTokenType.XML_NAME) {
        ...
        final int state = getState() & BASE_STATE_MASK;
        if (state == _TweeLexer.COMMENT) return XmlTokenType.XML_COMMENT_CHARACTERS;
      }
      return tokenType;
    }
    else {
